...
export default function Snowman({ position, onLoadComplete }: Props) {
  const loadCompleteCalled = useRef(false);
  const { scene, animations } = useGLTF('/models/snowman/snowman.gltf');
  ...
  return (
    <>
      {/* 캐릭터 렌더링 */}
      <primitive object={scene} scale={[2.5, 2.5, 2.5]} position={position} />
    </>
  );
}
