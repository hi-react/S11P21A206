...
export default function Santa({ position, onLoadComplete }: Props) {
  const loadCompleteCalled = useRef(false);
  const { scene, animations } = useGLTF('/models/santa/santa.gltf');
  ...
  return (
    <>
      {/* 캐릭터 렌더링 */}
      <primitive object={scene} scale={[6, 6, 6]} position={position} />
    </>
  );
}
